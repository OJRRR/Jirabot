...
def get_my_tasks() -> str:
    ...
    try:
        ...
        return json.dumps({"success": True, "count": len(tasks), "tasks": tasks},
...
def get_project_tasks(project_key: str = "") -> str:
    ...
    try:
        if not project_key:
            return json.dumps({"success": False, "error": "请提供 project_key 参数"},
                              ensure_ascii=False)
...
def search_issues(query: str = "", project: str = "", status: str = "",
    ...
    try:
        ...
        if not jql_parts:
            return json.dumps({"success": False, "error": "请至少提供一个搜索条件"},
                              ensure_ascii=False)
...
def get_create_issue_metadata(project_key: str = "",
                               issue_type_name: str = None) -> str:
    """获取在指定项目中创建问题的元数据，包括可用的问题类型及其必填/可选字段。
    参数：
      project_key - 项目KEY
      issue_type_name - 可选，按名称过滤问题类型
    结果会被缓存，同一项目+类型的组合不会重复请求 Jira API。
    """
    try:
        if not project_key:
            return json.dumps({"success": False, "error": "请提供 project_key"},
                              ensure_ascii=False)
        project_key = validate_project_key(project_key)
        ...
        if not raw or "projects" not in raw:
            return json.dumps({"success": False,
                               "error": f"无法获取项目 {project_key} 的元数据"},
                              ensure_ascii=False)
        ...
        result = json.dumps({"success": True, "data": {"project_key": project_key,
...
def get_issue_transitions(issue_key: str = "") -> str:
    ...
    参数：
      issue_key - 任务KEY（如 KO-29）
    返回 JSON 字符串，包含可用的转换目标和状态。
    """
    try:
        key = extract_issue_key(issue_key)
        if not key:
            return json.dumps({"success": False, "error": "请提供有效的 issue_key"},
                              ensure_ascii=False)
        transitions = jira.get_issue_transitions(key)
        return json.dumps({"success": True, "data": transitions},
                          ensure_ascii=False, indent=2)
    except Exception as e:
        return json.dumps({"success": False, "error": str(e)}, ensure_ascii=False)
...
def suggest_epic_tasks(epic_key: str = "") -> str:
    ...
    try:
        ...
        if not key:
            return json.dumps({"success": False, "error": "请提供有效的 Epic KEY"},
                              ensure_ascii=False)

        epic = jira.get_issue(key)
        if not epic:
            return json.dumps({"success": False, "error": f"未找到 {key}"},
                              ensure_ascii=False)
        ...
        return json.dumps({"success": True, "epic": result},
                          ensure_ascii=False, indent=2)
    except Exception as e:
        return json.dumps({"success": False, "error": str(e)}, ensure_ascii=False)
